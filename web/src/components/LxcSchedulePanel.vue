  ...
  <div class="lxc-schedule-panel">
    <div class="panel-toolbar">
      <el-alert
        ...
        title="可为当前容器设置一次性、每日或每周定时任务。删除容器属于高风险动作，仅支持一次性任务，并会触发二次验证。"
      />
      ...
      <el-form ref="formRef" :model="form" label-width="110px">
        <el-form-item label="事件类型">
          <el-radio-group v-model="form.eventType" @change="handleEventTypeChange">
            ...
            <el-radio-button label="lxc">容器事件</el-radio-button>
          </el-radio-group>
        </el-form-item>
        ...
        <el-alert
          ...
          :closable="false"
          show-icon
          title="删除容器会走异步任务队列执行。任务触发后会自动清理该容器关联的定时任务。"
        />
      </el-form>
...
const actionOptions = computed(() => {
  if (form.eventType === 'lxc') {
    return [{ value: 'delete', label: '删除此容器' }]
  }
  return [
    { value: 'start', label: '定时启动' },
    { value: 'stop', label: '定时停止' }
  ]
})
...
const fetchSchedules = async () => {
  ...
  loading.value = true
  try {
    const res = await getLXCSchedules(props.name)
...
const normalizeFormForEvent = () => {
  if (form.eventType === 'lxc') {
    ...
    if (!['start', 'stop'].includes(form.action)) {
      form.action = 'start'
    }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateLXCSchedule(props.name, editingId.value, payload)
      ElMessage.success('定时任务已更新')
    } else {
      await createLXCSchedule(props.name, payload)
...
const handleToggle = async (row, enabled) => {
  ...
  try {
    await updateLXCSchedule(props.name, row.id, buildPayloadFromRow(row, enabled))
    row.enabled = enabled
    if (!enabled) {
...
const handleDelete = async (row) => {
  ...
  await deleteLXCSchedule(props.name, row.id)
