...
// ensureNicConfig 规范化每张网卡的 config 块（writeConfig 每次写入前调）：
//   - 删除 lxc.net.<order>.link（app 完全接管 OVS，去 link 使 LXC 必然降级、不再原生 attach）
//   - 写 lxc.net.<order>.name = eth<order>（与 order 对齐，findContainerHostVeth 依赖）
//   - 写 lxc.net.<order>.veth.pair = <稳定名>（app 预建 OVS 端口用）
func ensureNicConfig(name string, blocks map[int]map[string]string) {
	for o, blk := range blocks {
		...
		blk["veth.pair"] = vethPairName(name, o)
	}
}
