	...
	api := r.Group("/api")
	{
		...
		authorized.Use(middleware.ForcePasswordChangeMiddleware())
		{
			...
			lxcGroup.Use(middleware.LXCAccessMiddleware())
			{
				...
				lxcGroup.GET("/:name/config-file", middleware.AdminMiddleware(), handler.GetLXCConfigFile)
				lxcGroup.PUT("/:name/config-file", middleware.AdminMiddleware(), handler.SetLXCConfigFile)
				...
				lxcGroup.PUT("/:name/cpu-limit", middleware.AdminMiddleware(), handler.SetLXCCPULimit)
				// LXC 目录挂载（仅管理员）
			...
			vpc := authorized.Group("/vpc")
			{
				...
				vpc.GET("/switches/:id/available-ips", handler.GetAvailableVPCIPs)
