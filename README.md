...
- **网络**：多网卡增删改（VPC 交换机 / 安全组），主网卡可绑定静态 IP；**端口映射**（复用宿主机 iptables DNAT，宿主端口转发到容器 IP，与 VM 共享配额，容器删除 / 解绑时自动清理）。
- **快照与克隆**：zfs / dir 双分支快照（带备注）、从快照克隆容器、被克隆的快照禁删保护。
- **监控**：CPU / 内存 / 网络 / 磁盘用量的实时与历史图表。
- **健康检查**：可配置探测规则（http / tcp / script），四色聚合（healthy / degraded / unhealthy / unknown）写入容器状态，列表页状态灯直观反映容器内服务真实健康（区别于「容器在跑但服务已僵死」）；后台调度器周期轮询（维护模式跳过、并发上限），支持手动立即探测。
- **定时任务**：定时启动 / 停止 / 删除（once / daily 等），并入调度事件中心。
- **Web 终端**：基于 lxc-attach 的浏览器终端。
- **配置文件编辑**（管理员）：直接查看 / 编辑容器原始 config 文件（monospace 编辑器，暴露 `lxc.cap.*` / `apparmor` / `idmap` / `hook` / `sysctl` 等高级键），保存前自动备份并保留最近 10 份、支持一键恢复；仅容器关机时可写。
...
**btrfs 存储管理（新增）**：与 ZFS / LVM 平级的独立存储后端（CoW、多设备、原生数据冗余），运维能力对齐 ZFS：

- 池创建（单盘 / 多盘，data profile：single / raid0 / raid1 / raid10），压缩开关（zstd / off），vm-disks 可一键关闭 CoW（nodatacow）提升随机写性能。
- 在线扩容（`btrfs device add`）与缩容移盘（`btrfs device delete`，严格护栏 + 异步任务）。
- Scrub 数据校验（启动 / 状态 / 取消，发现并修复静默错误）。
- Balance / 在线改 RAID（空间回收与 data/metadata profile 转换，严格护栏防降级）。
- 在线改属性（压缩算法 remount + fstab 持久化、vm-disks CoW 开关）。
